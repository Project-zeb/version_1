...
SQLITE_CONTINUOUS_SYNC_FROM_MYSQL = str(os.getenv("SQLITE_CONTINUOUS_SYNC_FROM_MYSQL", "1") or "1").strip().lower() in ["1", "true", "yes", "on"]
MYSQL_REVERSE_SYNC_FROM_SQLITE = str(os.getenv("MYSQL_REVERSE_SYNC_FROM_SQLITE", "1") or "1").strip().lower() in ["1", "true", "yes", "on"]
...
def mobile_live_alerts():
    ...
    if use_internal_source:
        ...
        if not formatted and source_policy == 'live_only':
            return jsonify({
                'success': False,
                'message': 'Live alerts unavailable in direct and internal modes',
                'source_mode': 'internal_unavailable',
